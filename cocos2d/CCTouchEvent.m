...
- (void)updateTouchesBegan:(NSSet*)touches
{
    ...
    {
        NSEnumerator* deadTouchIter = [_deadTouches objectEnumerator];

        CCTouch* ccTouch = [deadTouchIter nextObject];
        ccTouch.uiTouch = touch;
        
        [_allTouches setObject:ccTouch forKey:[NSValue valueWithPointer:(void*)touch]];
    ...
    for(PlatformTouch* touch in touches)
    {
        CCTouch* ccTouch = [_allTouches objectForKey:[NSValue valueWithPointer:(void*)touch]];
        if(ccTouch)
        {
...
- (void)updateTouchesEnded:(NSSet*)touches
{
    ...
    for(PlatformTouch* touch in touches)
    {
        ...
        [keys addObject:[NSValue valueWithPointer:(void*)touch]];
    }
